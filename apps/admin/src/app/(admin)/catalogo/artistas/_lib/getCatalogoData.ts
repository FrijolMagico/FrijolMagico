...
export async function getCatalogoArtistas(
  page: number = 1,
  filters: CatalogoFilters = { activo: null, destacado: null, search: '' }
  ...
  if (filters.search) {
    ...
    conditions.push(
      or(
        like(artista.nombre, searchTerm),
        like(artista.pseudonimo, searchTerm)
      )
    )
  }
  ...
  // Map results to CatalogoArtista
  const artistasWithMetadata: CatalogoArtista[] = results.map((row) => ({
