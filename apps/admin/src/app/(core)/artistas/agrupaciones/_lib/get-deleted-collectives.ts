...
import type { DeletedCollectiveRow } from '../_types/collective.types'

const { collective } = artist

function buildDeletedWhereClause(query: CollectiveQueryParams) {
...
export async function getDeletedCollectives(
  queryParams: unknown
): Promise<PaginatedResponse<DeletedCollectiveRow>> {
  'use cache'
  cacheTag(COLLECTIVE_DELETED_CACHE_TAG)
  ...
  return createPaginatedResponse(
    results.filter(
      (item): item is DeletedCollectiveRow =>
