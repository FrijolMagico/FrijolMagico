...
describe('FestivalesTimelineContent', () => {
  test('renders one card and detail link per festival with unique structural IDs', () => {
    render(
      <FestivalesTimelineContent festivales={mockFestivales} activeId={null} />
    )
  ...
  test('alternates desktop card placement and connector direction', () => {
    ...
    const secondCardWrapper = secondSection.querySelector(
      '[data-card-placement]'
    )
    const firstConnector = firstSection.querySelector(
    ...
    expect(firstSection.className).toContain(
      'min-[1360px]:landscape:grid-cols-2'
    )
    expect(firstSection.className).not.toContain('lg:landscape:grid-cols-2')
    expect(firstSection.className).not.toContain('lg:grid-cols-[40rem_40rem]')
    expect(firstSection.className).not.toContain(
      'lg:landscape:grid-cols-[40rem_40rem]'
    )
    expect(firstCardWrapper?.className).toContain(
      'min-[1360px]:landscape:col-start-1'
    )
    expect(secondCardWrapper?.className).toContain(
      'min-[1360px]:landscape:col-start-2'
    )
    expect(firstConnector?.getAttribute('data-connector-placement')).toBe(
      'right'
    )
    expect(secondConnector?.getAttribute('data-connector-placement')).toBe(
      'left'
    )
    expect(firstConnector?.firstElementChild?.className).toContain(
      '-translate-x-8'
    )
    expect(secondConnector?.firstElementChild?.className).toContain(
      'translate-x-8'
    )
  ...
  test('keeps cards vertical until the large landscape layout is active', () => {
    render(
      <FestivalesTimelineContent festivales={mockFestivales} activeId={null} />
    )
    ...
    expect(articles[0]?.className).toContain('lg:landscape:flex-row-reverse')
    expect(articles[0]?.className).toContain('md:flex-row')
    expect(articles[1]?.className).toContain('md:flex-row')
    expect(articles[1]?.className).not.toContain('lg:landscape:flex-row')
    expect(articles[1]?.className).not.toContain('lg:flex-row-reverse')
    expect(articles[0]?.firstElementChild?.className).toContain('w-full')
    expect(articles[0]?.firstElementChild?.className).not.toContain('min-w-0')
    expect(articles[0]?.firstElementChild?.className).not.toContain('flex-1')
  })
