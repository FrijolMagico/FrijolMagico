-- Migración: Hacer nombre nullable y pseudonimo NOT NULL
-- Además, poner NULL en nombre donde nombre = pseudonimo
...
-- 2. Vaciar tablas dependientes (para evitar FK violations)
...
-- 3. Crear nueva tabla artista con schema actualizado
CREATE TABLE artista_new (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    nombre TEXT,              -- ahora nullable
    pseudonimo TEXT NOT NULL, -- ahora NOT NULL
    correo TEXT,
    rrss TEXT,
    ciudad TEXT,
    descripcion TEXT,
...
-- 4. Copiar datos de artista (poniendo NULL donde nombre = pseudonimo)
INSERT INTO artista_new (id, nombre, pseudonimo, correo, rrss, ciudad, descripcion, pais, created_at, updated_at, slug)
SELECT 
    id,
    CASE WHEN LOWER(TRIM(nombre)) = LOWER(TRIM(pseudonimo)) THEN NULL ELSE nombre END,
    pseudonimo,
    correo, rrss, ciudad, descripcion, pais, created_at, updated_at, slug
FROM artista;

-- 5. Eliminar tabla original y renombrar
DROP TABLE artista;
